...
namespace WhereAreWe
{
    ...
    public enum Action
    {
        ...
        TeleportPartyS,
        TeleportPartySE,
        TeleportPartyForward,
        TeleportPartyBackward,
        TeleportPartyLeft,
        TeleportPartyRight,
        TeleportPartyForwardLeft,
        TeleportPartyForwardRight,
        TeleportPartyBackwardLeft,
        TeleportPartyBackwardRight,
    ...
    public class ResistanceValue : IComparable<ResistanceValue>
    {
        ...
        public int Bonus;
        public int Modifier;
        public string ReasonBase = "";
        public string ReasonBonus = "";
        public string ReasonModifier = "";
        ...
        public ResistanceValue(GenericResistanceFlags resist, int val, int bonus, int modifier, string reasonBase = "", string reasonBonus = "", string reasonModifier = "")
        {
            Resistance = resist;
            BaseValue = val;
            Bonus = bonus;
        ...
        public ResistanceValue(GenericResistanceFlags resist, int val, int bonus, string reasonBase = "", string reasonBonus = "") :
            this(resist, val, bonus, 0, reasonBase, reasonBonus, "") { }

        public ResistanceValue(GenericResistanceFlags resist, int val, string reason = "") :
            this(resist, val, 0, 0, reason, "", "") { }
    ...
    public static class Global
    {
        ...
        public static EOB1Globals EOB1 = new EOB1Globals();
        public static EOB2Globals EOB2 = new EOB2Globals();

        public static Ultima1Globals Ultima1 = new Ultima1Globals();
        ...
        public const char SpoilerStart = '«';
        public const char SpoilerEnd = '»';
        public static char[] SpoilerChars = new char[] { Global.SpoilerStart, Global.SpoilerEnd };
        ...
        public static BitmapSet BmpOneItem = BitmapsFromIcon(Properties.Resources.iconOneItem);
        public static BitmapSet BmpTwoItems = BitmapsFromIcon(Properties.Resources.iconTwoItems);
        public static BitmapSet BmpThreeItems = BitmapsFromIcon(Properties.Resources.iconThreeItems);
        ...
        public static BitmapSet BmpYouAreHereDirectionless = BitmapsFromIcon(Properties.Resources.iconYouAreHereDirectionless);
        ...
        public static BitmapSet GetItemBitmapSet(int iTotal)
        {
            if (iTotal > 2)
                return Global.BmpThreeItems;
            else if (iTotal > 1)
                return Global.BmpTwoItems;
            return Global.BmpOneItem;
        }
        ...
        public const string UnidentifiedItemNoTradeTip = "This item has not yet been identified.";
        public const string UnidentifiedItemTip = UnidentifiedItemNoTradeTip + "\r\n\r\nYou may trade it to another character by pressing that character's number.";
        ...
        public static string[] ItemFormats = new string[] {
            "$[Cursed] $[Broken] ($[Count]) $[Name] ($[Why]), $[Type] $[DamAC], $[Equip], $[Use] [$[Charges]], $[Value]",
        ...
        public static Direction DirectionForAction(Action action)
        {
            switch (action)
            {
        ...
        public static bool SafeRegexMatch(string strSource, string strPattern)
        {
            try
            {
                return Regex.IsMatch(strSource, strPattern);
            }
            catch (Exception ex)
            {
                Global.LogWarning("Invalid regex encountered: " + ex.Message);
                return false;
            }
        }

        public static Direction DirectionForAction(Action action, MemoryHacker hacker)
        {
            GameState state = hacker.GetGameState();
            if (state == null)
                return Direction.None;

            Direction dir = state.Location.Facing;
            
            switch (action)
            {
                case Action.TeleportPartyForward: return dir;
                case Action.TeleportPartyBackward: return Global.Opposite(dir);
                case Action.TeleportPartyLeft: return Global.Left(dir);
                case Action.TeleportPartyRight: return Global.Right(dir);
                case Action.TeleportPartyForwardLeft: return Global.HalfLeft(dir);
                case Action.TeleportPartyForwardRight: return Global.HalfRight(dir);
                case Action.TeleportPartyBackwardLeft: return Global.HalfRight(Global.Opposite(dir));
                case Action.TeleportPartyBackwardRight: return Global.HalfLeft(Global.Opposite(dir));
                default: return Direction.None;
            }
        }
        ...
        public static int FindFirstBytes(byte[] searchIn, byte[] searchFor, int iStart = 0)
        {
        ...
        public static int[] FindBytes(byte[] searchIn, byte[] searchFor, int iStart = 0)
        {
            ...
            do
            {
                iStart = FindFirstBytes(searchIn, searchFor, iStart);
                if (iStart != -1)
                {
                    found.Add(iStart);
                    iStart += (searchFor.Length - 1);
                }
            } while (iStart != -1);
        ...
        public static Direction HalfRight(Direction dir)
        {
            switch (dir)
            {
        ...
        public static Direction HalfLeft(Direction dir)
        {
            switch (dir)
            {
        ...
        public static void SetSplitterDistance(SplitContainer sc, int[] positions, int index)
        {
            if (positions == null || index >= positions.Length)
                return;
            SetSplitterDistance(sc, positions[index]);
        }
        ...
        public static string SingleResistance(GenericResistanceFlags resist)
        {
            switch (resist)
            {
                ...
                case GenericResistanceFlags.SaveVsBreath: return "Breath";
                case GenericResistanceFlags.SaveVsParalyze: return "Paralyze";
                case GenericResistanceFlags.SaveVsPetrify: return "Petrify";
                case GenericResistanceFlags.SaveVsPoison: return "Poison";
                case GenericResistanceFlags.SaveVsSleep: return "Sleep";
                case GenericResistanceFlags.SaveVsSpell: return "Spell";
                case GenericResistanceFlags.SaveVsWand: return "Wand";
                default: return "Unknown";
            }
        }

        public static string GetNullTerminatedString(byte[] bytes, int index, int max, bool bTrim = true)
        {
            int iLength = 0;
            ...
            string strResult = Encoding.ASCII.GetString(bytes, index, iLength);
            return bTrim ? strResult.Trim() : strResult;
        }

        public static List<string> GetNullTerminatedStrings(byte[] bytes, int index, int max)
        {
            List<string> list = new List<string>();

            int iStart = 0;
            int iLength = 0;
            while (iStart + iLength < max)
            {
                while (bytes[index + iStart + iLength] != 0)
                {
                    iLength++;
                    if (iStart + iLength >= max)
                        break;
                }
                if (iStart + iLength >= max)
                    break;
                list.Add(Encoding.ASCII.GetString(bytes, index + iStart, iLength));
                iStart += iLength;
                iLength = 0;
            }
        ...
        public static string ToHex(byte[] bytes, int iStart = 0, int iLength = -1)
        {
            if (bytes == null)
                return "<null>";
            if (iLength == -1)
        ...
        public static long ConvertBytesToLong(byte[] bytes, int iIndex = 0)
        {
            ...
            if (bytes.Length - iIndex >= 5)
            {
                byte[] bytes8 = NullBytes(8);
                for (int i = 0; i < bytes.Length; i++)
                    bytes8[i] = bytes[i];
                return BitConverter.ToInt64(bytes8, iIndex);
            }
            if (bytes.Length - iIndex >= 4)
                return BitConverter.ToInt32(bytes, iIndex);
            if (bytes.Length - iIndex >= 3)
            {
                byte[] bytes4 = NullBytes(4);
                for (int i = 0; i < bytes.Length; i++)
                    bytes4[i] = bytes[i];
                return BitConverter.ToInt32(bytes4, iIndex);
            }
        ...
        public static void UpdateBonusTable(ListView lvBonusTable, MemoryHacker hacker, PrimaryStat stat, int iMaxStat = -1)
        {
            ...
            do
            {
                ...
                if (mod.Next == -1)
                    ...
                    lvi = new ListViewItem(String.Format("{0}", i));
                lvi.SubItems.Add(mod.PlusValue);
        ...
        {
            if (bytes == null || iStart > bytes.Length || iStart < 0)
                return null;
            if (iLength == -1)
                iLength = bytes.Length - iStart;
        ...
        public static byte[] Clone(byte[] bytes) => Subset(bytes, 0);
        ...
        public static string ActionString(Action action)
        {
            switch (action)
            {
                ...
                case Action.TeleportPartyNW: return "Teleport the party one square NW";
                case Action.TeleportPartyN: return "Teleport the party one square N";
                case Action.TeleportPartyNE: return "Teleport the party one square NE";
                case Action.TeleportPartyE: return "Teleport the party one square E";
                case Action.TeleportPartySE: return "Teleport the party one square SE";
                case Action.TeleportPartyS: return "Teleport the party one square S";
                case Action.TeleportPartySW: return "Teleport the party one square SW";
                case Action.TeleportPartyW: return "Teleport the party one square W";
                case Action.TeleportPartyForward: return "Teleport the party forward";
                case Action.TeleportPartyBackward: return "Teleport the party backward";
                case Action.TeleportPartyLeft: return "Teleport the party left";
                case Action.TeleportPartyRight: return "Teleport the party right";
                case Action.TeleportPartyForwardLeft: return "Teleport the party forward/left";
                case Action.TeleportPartyForwardRight: return "Teleport the party forward/right";
                case Action.TeleportPartyBackwardLeft: return "Teleport the party backward/left";
                case Action.TeleportPartyBackwardRight: return "Teleport the party backward/right";
        ...
        public static void InsertText(RichTextBox tb, string str)
        {
        ...
        public static byte[] BytesFor(long offset, int iCount, bool bReverse = false)
        {
            byte[] bVal = BitConverter.GetBytes(offset);
            byte[] bytesOut = NullBytes(iCount);
            for (int i = 0; i < iCount; i++)
            {
                if (i >= bVal.Length)
                    break;
                if (bReverse)
                    bytesOut[iCount - i] = bVal[i];
                else
                    bytesOut[i] = bVal[i];
            }
            return bytesOut;
        ...
        {
            if (str == null || str.Length < 1 || !Char.IsLower(str[0]))
                return str;
        ...
        public static void SelectOnly(ListView lv, ListViewItem lvi, bool bBeginEndUpdate = true)
        {
            if (bBeginEndUpdate)
                lv.BeginUpdate();
            int[] indices = new int[lv.SelectedIndices.Count];
            lv.SelectedIndices.CopyTo(indices, 0);
            foreach (int i in indices)
                lv.Items[i].Selected = (lv.Items[i] == lvi);        // Don't de-select and re-select
            if (lvi != null)
                lvi.Selected = true;
            if (bBeginEndUpdate)
                lv.EndUpdate();
        ...
        public static GenericClass[] SeparateClasses(GenericClass gc)
        {
            switch (gc)
            {
                case GenericClass.FighterCleric: return new GenericClass[] { GenericClass.Fighter, GenericClass.Cleric };
                case GenericClass.FighterThief: return new GenericClass[] { GenericClass.Fighter, GenericClass.Thief };
                case GenericClass.FighterMage: return new GenericClass[] { GenericClass.Fighter, GenericClass.Mage };
                case GenericClass.FighterMageThief: return new GenericClass[] { GenericClass.Fighter, GenericClass.Mage, GenericClass.Thief };
                case GenericClass.ThiefMage: return new GenericClass[] { GenericClass.Thief, GenericClass.Mage };
                case GenericClass.ClericThief: return new GenericClass[] { GenericClass.Cleric, GenericClass.Thief };
                case GenericClass.FighterClericMage: return new GenericClass[] { GenericClass.Fighter, GenericClass.Cleric, GenericClass.Mage };
                case GenericClass.RangerCleric: return new GenericClass[] { GenericClass.Ranger, GenericClass.Cleric };
                case GenericClass.ClericMage: return new GenericClass[] { GenericClass.Cleric, GenericClass.Mage };
                default: return new GenericClass[] { gc };
            }
        }

        public static void MoveControls(int dx, int dy, params Control[] ctrls)
        {
            foreach (Control ctrl in ctrls)
                ctrl.Location = new Point(ctrl.Location.X + dx, ctrl.Location.Y + dy);
        }

        public static void UpdateSubItem(ListViewItem lvi, int iSubIndex, string str)
        {
            if (lvi == null || iSubIndex < 0)
                return;
            if (lvi.SubItems.Count >= iSubIndex + 1)
            {
                lvi.SubItems[iSubIndex].Text = str;
                return;
            }
            while (lvi.SubItems.Count < iSubIndex)
                lvi.SubItems.Add("");
            lvi.SubItems.Add(str);
        }

        public static string CollapseSequences(List<string> list)
        {
            // Example input:  a; b; c; c; c; d; e; d; e; d; a; e
            // Example output: a; b; c x3; (d; e) x2; d; a; e
            StringBuilder sb = new StringBuilder();
            int i = 0;
            while (i < list.Count)
            {
                int iDupSingle = 0;
                int iDupDouble = 0;
                while (iDupSingle + i + 1 < list.Count && list[i] == list[iDupSingle + i + 1])
                    iDupSingle++;
                if (iDupSingle > 0)
                    sb.AppendFormat("{0} x{1}; ", list[i], iDupSingle + 1);
                else
                {
                    while (iDupDouble + i + 3 < list.Count && list[i] == list[iDupDouble + i + 2] && list[i + 1] == list[iDupDouble + i + 3])
                        iDupDouble += 2;
                    if (iDupDouble > 0)
                        sb.AppendFormat("({0}; {1}) x{2}; ", list[i], list[i+1], (iDupDouble + 2) / 2);
                    else
        ...
        public static bool HashEquals<T>(HashSet<T> hash1, HashSet<T> hash2)
        {
        ...
        // Adds together a series of Int64 values (as strings)
        public static bool SumInt64(List<string> values, out Int64 sum)
        {
            sum = 0;
            bool bValid = false;    // True if any of the values are integers; non-ints are skipped
            foreach (string str in values)
            {
                if (Int64.TryParse(str, out Int64 iResult))
                {
                    bValid = true;
                    sum += iResult;
                }
        ...
        public static bool SaveTextWithRetry(string sFileName, string sSaveText, string sErrorTitle = "Error Saving to File")
        {
            while (true)
            {
                try
                {
                    File.WriteAllText(sFileName, sSaveText);
                    return true;
                }
                catch (Exception ex)
                {
                    if (MessageBox.Show("The file \"" + sFileName + "\" could not be written.\r\n\r\nException:  " + ex.Message, sErrorTitle, MessageBoxButtons.RetryCancel, MessageBoxIcon.Error) != DialogResult.Retry)
                        return false;
                }
            }
        }

        public static string LoadTextWithRetry(string sFileName, string sErrorTitle = "Error Reading from File")
        {
            while (true)
            {
                try
                ...
                {
                    if (MessageBox.Show("The file \"" + sFileName + "\" could not be read.\r\n\r\nException:  " + ex.Message, sErrorTitle, MessageBoxButtons.RetryCancel, MessageBoxIcon.Error) != DialogResult.Retry)
                        return null;
                }
            }
        }

        public static Color GetGradientValue(Color c1, Color c2, long mid, long min, long max)
        {
            int rMax = c2.R;
            int rMin = c1.R;
            int gMax = c2.G;
            ...
            int rAverage = rMin + (int)((rMax - rMin) * point / range);
            int gAverage = gMin + (int)((gMax - gMin) * point / range);
            int bAverage = bMin + (int)((bMax - bMin) * point / range);

            return Color.FromArgb(rAverage, gAverage, bAverage);
        }

        public static void SetDefaultStyle(ListView lv, Font font, Color fore, Color back)
        {
            lv.BeginUpdate();
            foreach (ListViewItem lvi in lv.Items)
            {
        ...
        public static string FormatXml(string xml)
        {
            ...
            {
                // Something went wrong; just return the unformatted string as a fallback
                return xml;
            }
        }
        ...
        public static string RtfColorString(Color c)
        {
            return String.Format(@"\red{0}\green{1}\blue{2}", c.R, c.G, c.B);
        }

        public static string FormatForSingleLine(string text)
        {
            if (text.IndexOf('\n') == -1)
                return text;
            ...
            sb.Replace("\r\n", "; ");
            sb.Replace("\n", "; ");
            while (sb.Length >= 2 && sb[sb.Length - 2] == ';' && sb[sb.Length - 1] == ' ')
                sb.Remove(sb.Length - 2, 2);
            return sb.ToString();
        }
    }
    ...
    public class UInt24 : IConvertible
    {
        private int m_value;
        ...
        public bool ToBoolean(IFormatProvider provider) => m_value != 0;
        public char ToChar(IFormatProvider provider) { throw new NotImplementedException(); }
        public sbyte ToSByte(IFormatProvider provider) => (sbyte)m_value;
        public byte ToByte(IFormatProvider provider) => (byte)m_value;
        public short ToInt16(IFormatProvider provider) => (Int16)m_value;
        public ushort ToUInt16(IFormatProvider provider) => (UInt16)m_value;
        public int ToInt32(IFormatProvider provider) => (Int32)m_value;
        public uint ToUInt32(IFormatProvider provider) => (UInt32)m_value;
        public long ToInt64(IFormatProvider provider) => m_value;
        public ulong ToUInt64(IFormatProvider provider) => (ulong) m_value;
        public float ToSingle(IFormatProvider provider) => (Single)m_value;
        public double ToDouble(IFormatProvider provider) => (Double)m_value;
        public decimal ToDecimal(IFormatProvider provider) => (Decimal)m_value;
        public DateTime ToDateTime(IFormatProvider provider) => DateTime.FromFileTime(m_value);
        public string ToString(IFormatProvider provider) => m_value.ToString();
        public object ToType(Type conversionType, IFormatProvider provider) => Convert.ChangeType(m_value, conversionType);
    ...
    public class DamageDice : IComparable<DamageDice>
    {
        ...
        public DamageDice Plus(int iBonus) { return new DamageDice(this, Bonus + iBonus); }
    }
    ...
    public class InventoryItemTag
    {
        ...
        public string FormatForDisplay()
        {
            ...
            return String.Format("{0} {1}", DisplayIndex, ListViewString);
        }
    }
    ...
    public class MapXY
    {
        ...
        public MapXY(EOB1Map map, int x, int y) : this(GameNames.EyeOfTheBeholder1, (int)map, x, y) { }
        public MapXY(EOB2Map map, int x, int y) : this(GameNames.EyeOfTheBeholder2, (int)map, x, y) { }
        public MapXY(Ultima1Map map, int x, int y) : this(GameNames.Ultima1, (int)map, x, y) { }
    ...
    public class Modifiers
    {
        public static ModAttr GetAttrib(GenericAttribute attrib)
        {
            switch (attrib)
            {
                ...
                case GenericAttribute.Strength18: return ModAttr.Strength18;
                case GenericAttribute.Stamina: return ModAttr.Stamina;
        ...
        public int Intelligence { get { return Value(ModAttr.Intelligence); } }
        public int Wisdom { get { return Value(ModAttr.Wisdom); } }
        public int Charisma { get { return Value(ModAttr.Charisma); } }
        public int Strength18 { get { return Value(ModAttr.Strength18); } }
    ...
    public class StatModifier
    {
        ...
        public string PlusValue { get { return Global.AddPlus(Value); } }

        public static StatModifier FromSubStat(int iMain, int iSub, int iValue, int iNext)
        {
            StatModifier sm = new StatModifier(iMain, iValue, iNext);
            sm.SubStat = iSub;
            return sm;
        }

        public void SetValueNext(int iValue, int iNext)
        {
            Value = iValue;
            Next = iNext;
        }
        ...
        {
            return String.Format(strFormat, Stat, bPlus ? PlusValue : Value.ToString(), NextString);
        }
        ...
        public string NextString { get { return NextBonus(Next); } }

        public static string NextBonus(int iNext)
        {
            if (iNext == -1)
                return "maximum bonus";
            return String.Format("next bonus at {0}", iNext);
        }
    }
    ...
    public class MapLineInfo
    {
        ...
        public static MapLineInfo BlackDash2 = new MapLineInfo(Color.Black, DashStyle.Dash, 2);
            ...
            public bool Test(GameNames game, MapSheet sheet, int x, int y)
            {
                ...
                foreach (LineSet set in Sets)
                {
                    ...
                    foreach (LinePair pair in set.Pairs)
                    {
                        ...
                        bool bFront = squareFront != null && squareFront.HasSightBlockingLine(game, sheet, pair.Front.Dir, sheet.GridLines, false);
                        bool bBack = squareBack != null && squareBack.HasSightBlockingLine(game, sheet, pair.Back.Dir, sheet.GridLines, true);
