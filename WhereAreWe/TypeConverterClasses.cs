...
namespace WhereAreWe
{
    ...
    public class MemoryGuesses
    {
        ...
        public static Dictionary<GameNames, MemoryGuess[]> GetDefaults()
        {
            ...
            foreach (GameNames game in Games.WizardryGames)
                guesses.Add(game, new MemoryGuess[] {
                    new MemoryGuess(1000000, 8066),
                    new MemoryGuess(1000000, 7514)
                });
            guesses.Add(GameNames.BardsTale1, new MemoryGuess[] {
                    new MemoryGuess(1000000, 154334),
                    new MemoryGuess(1000000, 153966)
                });
            guesses.Add(GameNames.BardsTale2, new MemoryGuess[] {
                    new MemoryGuess(1000000, 243687),
                    new MemoryGuess(1000000, 244055)
                });
            guesses.Add(GameNames.BardsTale3, new MemoryGuess[] {
                    new MemoryGuess(1000000, 213457),
                    new MemoryGuess(1000000, 213089)
                });
            guesses.Add(GameNames.EyeOfTheBeholder1, new MemoryGuess[] {
                    new MemoryGuess(1000000, 6626),
                    new MemoryGuess(1000000, 6384)
                });
            guesses.Add(GameNames.EyeOfTheBeholder2, new MemoryGuess[] {
                    new MemoryGuess(1000000, 6626),
                    new MemoryGuess(1000000, 6384)
                });
            return guesses;
    ...
    public class ColumnHeaderList
    {
        ...
        public ColumnHeaderList(ListView lv, int iSortColumn, bool bAscending)
        {
        ...
        public ColumnHeaderList(string str)
        {
            ...
            for (int i = 0; i < args.Length; i += 2)
            {
                ...
                {
                    SortAscending = args[i].EndsWith("+");
                    SortColumn = i / 2;
                    if (Int32.TryParse(args[i].Substring(0, args[i].Length-1), out iTest))
                        Widths.Add(iTest);
                }
        ...
        private string SortString(int iColumn)
        {
        ...
        public override string ToString()
        {
            ...
            {
                if (i >= Order.Count)
                    sb.AppendFormat("{0}{1},{2},", Widths[i], SortString(iOrderDefault), iOrderDefault);
                else
                    sb.AppendFormat("{0}{1},{2},", Widths[i], SortString(Order[i]), Order[i]);
                iOrderDefault++;
            }
    ...
    {
        public static ColorPattern DefaultEmpty = new ColorPattern(Color.White, HatchStyle.Percent05);
    ...
    [TypeConverter(typeof(ScriptViewerSettingsTypeConverter))]
    public class ScriptViewerSettings
    {
        public bool HideOffMap;
        public bool HideEmptyScripts;
        public bool HideInlineSubscripts;
        public bool HideEmptyLines;
        public bool HideUnreachableCode;
        public bool HideAddresses;

        public static ScriptViewerSettings FromXml(string xml)
        {
            ScriptViewerSettings sv = new ScriptViewerSettings();
            try
            {
                XmlDocument doc = new XmlDocument();
                doc.LoadXml(xml);
                XmlElement element = doc.SelectSingleNode("/ScriptViewer/Hide") as XmlElement;
                sv.HideOffMap = ReadBool(element, "offmap");
                sv.HideEmptyScripts = ReadBool(element, "emptyscripts");
                sv.HideInlineSubscripts = ReadBool(element, "inlinesubscripts");
                sv.HideEmptyLines = ReadBool(element, "emptylines");
                sv.HideUnreachableCode = ReadBool(element, "unreachablecode");
                sv.HideAddresses = ReadBool(element, "addresses");
            }
            catch (Exception ex)
            {
                Global.LogError("Invalid ScriptViewerSettings from string \"{0}\": {1}", xml, ex.Message);
            }
            return sv;
        }

        private static bool ReadBool(XmlElement element, string strName) { return element.HasAttribute(strName) ? Convert.ToInt32(element.Attributes[strName].Value) != 0 : false; }
        private static void WriteBool(XmlWriter writer, string strName, bool bVal) { writer.WriteAttributeString(strName, bVal ? "1" : "0"); }
        ...
        public override string ToString()
        {
            ...
            XmlWriter writer = XmlWriter.Create(sb);
            writer.WriteStartDocument();
            writer.WriteStartElement("ScriptViewer");
            writer.WriteStartElement("Hide");
            WriteBool(writer, "offmap", HideOffMap);
            WriteBool(writer, "emptyscripts", HideEmptyScripts);
            WriteBool(writer, "inlinesubscripts", HideInlineSubscripts);
            WriteBool(writer, "emptylines", HideEmptyLines);
            WriteBool(writer, "unreachablecode", HideUnreachableCode);
            WriteBool(writer, "addresses", HideAddresses);
            writer.WriteEndElement();
            writer.WriteEndElement();
            writer.WriteEndDocument();
            writer.Close();
    ...
    public class ScriptViewerSettingsTypeConverter : TypeConverter
    {
        public override bool CanConvertFrom(ITypeDescriptorContext context, Type sourceType)
        {
            return sourceType == typeof(string) || base.CanConvertFrom(context, sourceType);
        ...
        public override object ConvertFrom(ITypeDescriptorContext context, CultureInfo culture, object value)
        {
            ...
            {
                if ((string)value == "Default")
                    return new ScriptViewerSettings();

                return ScriptViewerSettings.FromXml(value as string);
            }
            return base.ConvertFrom(context, culture, value);
        }
        ...
        public override object ConvertTo(ITypeDescriptorContext context, CultureInfo culture, object value, Type destinationType)
        {
            if (destinationType == typeof(string))
            {
                return ((ScriptViewerSettings)value).ToString();
            }
            return base.ConvertTo(context, culture, value, destinationType);
        }
    }
