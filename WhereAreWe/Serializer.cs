    ...
    public class Serializer
    {
        ...
        public void Save(MapBook book, string strFile)
        {
            ...
            foreach (MapSheet sheet in book.Sheets)
            {
                ...
                if (sheet.Live)
                    nodeSheet.Attributes.Append(doc.CreateAttribute("live")).Value = "1";
                if (sheet.Directionless)
                    nodeSheet.Attributes.Append(doc.CreateAttribute("directionless")).Value = "1";
        ...
        public MapBook Load(string strFile)
        {
            ...
            foreach (XmlNode nodeSheet in root.SelectNodes("Sheet"))
            {
                if (nodeSheet is XmlElement)
                {
                    ...
                    string strSheetLive = String.Empty;
                    string strSheetDirectionless = String.Empty;
                    ...
                    XmlNode nodeSheetLive = nodeSheet.SelectSingleNode("Live");
                    if (nodeSheetLive != null)
                        strSheetLive = nodeSheetLive.InnerText;

                    strSheetDirectionless = e.Attributes["directionless"]?.Value;
                    strSheetLive = e.Attributes["live"]?.Value;
                    ...
                    sheet.Directionless = strSheetDirectionless == "1";
                    sheet.SheetNote = strSheetNote;
                    sheet.Live = strSheetLive == "1";
