...
{
    public partial class EOBCreationAssistantControl : CreationAssistantControl
    {
        ...
        private List<EOBDiceRolls> m_prevRolls;
        ...
        public EOBCreationAssistantControl()
        {
            InitializeComponent();
        ...
        private void Init()
        {
            m_prevRolls = new List<EOBDiceRolls>();
            NativeMethods.SetTooltipDelay(lvStats, 32000);
        }

        public EOBCreationAssistantControl(IMain main)
        {
            InitializeComponent();
            m_iOrigClassHeight = gbClasses.Height;
        ...
        private string StatRange(int iMin) { return String.Format("{0}-{1}", iMin, Math.Min(iMin + (IsEOB3 ? 7 : 6), (IsEOB3 ? 19 : 18))); }
        ...
        {
            return new EOBCharCreationInfo();
        }
            ...
            if (!m_bAcceptNewValues)
            {
                ...
                if (!bStatsSame)
                {
                    if (m_bIgnoreNextRoll)
                        ...
                        AddPreviousRollToList(new EOBDiceRolls(info.AttributesOriginal, info.Race));
        ...
        ListViewItem GetRollsLVI(EOBDiceRolls rolls)
        {
        ...
        private void AddPreviousRollToList(EOBDiceRolls rolls)
        {
            // Don't add this roll if it's exactly the same as the previous one in the list
            if (lvPrevious.Items.Count > 0)
            {
                EOBDiceRolls lastRolls = (EOBDiceRolls)lvPrevious.Items[lvPrevious.Items.Count - 1].Tag;
        ...
        private void UpdateItem(ListViewItem lvi, EOBDiceRolls rolls)
        {
        ...
        private bool IsEOB3 { get { return Hacker != null && Hacker.Game == GameNames.BardsTale3; } }
        ...
        private void AdjustRolls(GenericRace raceFrom, GenericRace raceTo)
        {
            ...
            {
                EOBDiceRolls rolls = lvPrevious.Items[i].Tag as EOBDiceRolls;
                if (rolls == null)
                ...
                Global.FixRange(ref rolls.Strength, 3, (byte) (IsEOB3 ? 19 : 18));
        ...
        private void LoadRolls(EOBDiceRolls rolls)
        {
            if (Hacker == null || !Hacker.IsValid)
                return;

            EOBCharCreationInfo info = Hacker.GetCharCreationInfo() as EOBCharCreationInfo;
            if (info == null)
                return;
            ...
            EOBCharCreationInfo infoNew = CreateInfo() as EOBCharCreationInfo;
        ...
        private void lvPrevious_ColumnClick(object sender, ColumnClickEventArgs e)
        {
            ...
            lvPrevious.ListViewItemSorter = new EOBDiceRollsComparer(lvPrevious, e.Column, m_bAscendingSort);
            lvPrevious.Sort();
        }
        ...
        private void DeleteLowerRaw()
        {
            ...
            EOBDiceRolls rolls = (EOBDiceRolls)lvPrevious.FocusedItem.Tag;
        ...
        private void ShowStats()
        {
            ...
            foreach (ListViewItem lvi in lvPrevious.Items)
            {
                EOBDiceRolls rolls = lvi.Tag as EOBDiceRolls;
                if (rolls == null)
                    continue;
        ...
        {
            return MinStat_EOB2(race, stat) - (IsEOB3 ? 1 : 0);
        }

        private int MinStat_EOB2(GenericRace race, PrimaryStat stat)
        {
            switch (race)
            {
        ...
        private int EOBRandomStat(int iMin)
        {
            if (IsEOB3)
        ...
        private int EOBRandomHitPoints()
        {
            if (IsEOB3)
        ...
        private void GenerateRolls(int iNum)
        {
            ...
            EOBDiceRolls[] allRolls = new EOBDiceRolls[iNum];

            for (int i = 0; i < iNum; i++)
            {
                byte[] bytes = new byte[6];
                bytes[0] = (byte)EOBRandomStat(MinStat(m_lastInfo.Race, PrimaryStat.Strength));
                bytes[1] = (byte)EOBRandomStat(MinStat(m_lastInfo.Race, PrimaryStat.IQ));
                bytes[2] = (byte)EOBRandomStat(MinStat(m_lastInfo.Race, PrimaryStat.Dexterity));
                bytes[3] = (byte)EOBRandomStat(MinStat(m_lastInfo.Race, PrimaryStat.Constitution));
                bytes[4] = (byte)EOBRandomStat(MinStat(m_lastInfo.Race, PrimaryStat.Luck));
                bytes[5] = (byte) EOBRandomHitPoints();
                allRolls[i] = new EOBDiceRolls(bytes, m_lastInfo.Race);
            }
        ...
        {
            if (lvPrevious.FocusedItem != null)
                LoadRolls((EOBDiceRolls)lvPrevious.FocusedItem.Tag);
        }
        ...
        private void RemoveByMinimumStat(int iMinWeight, int iMinRaw, int[] minStats)
        {
            ...
            {
                EOBDiceRolls rolls = (EOBDiceRolls)lvPrevious.Items[i].Tag;
        ...
        private void lvPrevious_DoubleClick(object sender, EventArgs e)
        {
            ...
            LoadRolls(lvPrevious.SelectedItems[0].Tag as EOBDiceRolls);
        }
    ...
    public class EOBDiceRolls
    {
        ...
        public EOBDiceRolls(byte[] attributes, GenericRace race)
        {
        ...
        {
            if (!(obj is EOBDiceRolls))
                return false;

            EOBDiceRolls rolls = obj as EOBDiceRolls;
    ...
    class EOBDiceRollsComparer : BasicListViewComparer
    {
        public EOBDiceRollsComparer(ListView lv, int column, bool bAscending) : base(lv, column, bAscending) { }

        public override int Compare(object x, object y)
        {
            EOBDiceRolls rolls1 = ((ListViewItem)x).Tag as EOBDiceRolls;
            EOBDiceRolls rolls2 = ((ListViewItem)y).Tag as EOBDiceRolls;

            switch (m_column)
            {
                case 0: return Order(Math.Sign(rolls1.RawTotal - rolls2.RawTotal));
                case 1: return Order(Math.Sign(rolls1.Strength - rolls2.Strength));
                case 2: return Order(Math.Sign(rolls1.IQ - rolls2.IQ));
                case 3: return Order(Math.Sign(rolls1.Dexterity - rolls2.Dexterity));
                case 4: return Order(Math.Sign(rolls1.Constitution - rolls2.Constitution));
                case 5: return Order(Math.Sign(rolls1.Luck - rolls2.Luck));
                case 6: return Order(Math.Sign(rolls1.HP - rolls2.HP));
