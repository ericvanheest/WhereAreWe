    ...
    public partial class WizardForm : Form
    {
        ...
        private void FindGamePaths()
        {
            ...
            foreach(GameNames game in Games.ImplementedGames)
                AddFoundGamePath(FindGamePath(game, Games.GetRegistries(game), Games.GetDefaultLink(game)));
        ...
        public static GamePathTag FindGamePath(GameNames game, string[] regEntries, string strLink)
        {
            foreach (string strReg in regEntries)
            {
        ...
        private void WizardForm_FormClosing(object sender, FormClosingEventArgs e)
        {
            ...
            if (m_bUpdatePaths)
            {
                List<GamePathTag> listFailed = new List<GamePathTag>();
                ...
                if (listFailed.Count > 0)
                {
                    ...
                    foreach (GamePathTag tag in listFailed)
                        sb.AppendFormat("{0} (\"{1}\"): {2}\r\n", Games.ShortName(tag.Game), tag.Path, tag.LastError);
                    ViewInfoForm.Show(String.Format("These DOSBox config files could not be automatically modified:\r\n\r\n{0}" +
                            "\r\nYou may wish to manually set them to windowed mode\r\n\r\n", sb.ToString()),
                            "Error modifying DOSBox config files");
                }
