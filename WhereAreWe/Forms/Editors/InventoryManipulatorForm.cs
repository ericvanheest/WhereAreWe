...
namespace WhereAreWe
{
    public partial class InventoryManipulatorForm : HackerBasedForm
    {
        ...
        private void lvBag_ColumnClick(object sender, ColumnClickEventArgs e)
        {
            ...
            lvBag.ListViewItemSorter = new BagItemComparer(lvBag, e.Column, m_bAscendingSort);
            lvBag.Sort();
        }
        ...
        private void BrowseForRosterFile()
        {
        ...
        private void miBackpackBrowseForRosterFile_Click(object sender, EventArgs e)
        {
            BrowseForRosterFile();
        }

        private void miBagBrowseForRosterFile_Click(object sender, EventArgs e)
        {
            BrowseForRosterFile();
        }
    }

    class BagItemComparer : BasicListViewComparer
    {
        public BagItemComparer(ListView lv, int column, bool bAscending) : base(lv, column, bAscending) { }
        ...
        public override int Compare(object x, object y)
        {
            ...
            switch (m_column)
            {
                case 0: return Order(String.Compare(item1.DescriptionString, item2.DescriptionString));
                case 1: return Order(String.Compare(item1.TypeString, item2.TypeString));
                case 2: return Order(String.Compare(item1.UsableString, item2.UsableString));
                case 3: return Order(String.Compare(item1.UsableByAlignment, item2.UsableByAlignment));
                case 4:
                    int iResult = String.Compare(item1.LargestBonusEffect, item2.LargestBonusEffect);
                    if (iResult == 0)
                        return Order(Math.Sign(item1.LargestBonus - item2.LargestBonus));
                    return Order(iResult);
