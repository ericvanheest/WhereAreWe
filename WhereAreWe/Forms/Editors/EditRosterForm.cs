...
namespace WhereAreWe
{
    public partial class EditRosterForm : CommonKeyForm
    {
        ...
        private void lvCharacters_ColumnClick(object sender, ColumnClickEventArgs e)
        {
            ...
            lvCharacters.ListViewItemSorter = new RosterItemComparer(lvCharacters, e.Column, m_bAscendingSort);
            lvCharacters.Sort();
            SetDirty();
        }
    ...
    class RosterItemComparer : BasicListViewComparer
    {
        public RosterItemComparer(ListView lv, int column, bool bAscending) : base(lv, column, bAscending) { }
        ...
        public override int Compare(object x, object y)
        {
            ...
            {
                case 0:
                    return Order(String.Compare(cab1.Char.Name, cab2.Char.Name));
                case 1:
                    return Order(Math.Sign(cab1.Position - cab2.Position));
                case 2:
                    return Order(String.Compare(((ListViewItem)x).SubItems[m_column].Text, ((ListViewItem)y).SubItems[m_column].Text));
                case 3:
                    return Order(Math.Sign(cab1.Char.BasicLevel.Permanent - cab2.Char.BasicLevel.Permanent));
                case 4:
                    return Order(String.Compare(((ListViewItem)x).SubItems[m_column].Text, ((ListViewItem)y).SubItems[m_column].Text));
                case 5:
                    return Order(String.Compare(((ListViewItem)x).SubItems[m_column].Text, ((ListViewItem)y).SubItems[m_column].Text));
                case 6:
                    return Order(Math.Sign(cab1.Char.QuickRefHitPoints.Maximum - cab2.Char.QuickRefHitPoints.Maximum));
                case 7:
                    if (cab1.Char.QuickRefSpellPoints is MMSpellPoints)
                        return Order(Math.Sign(((MMSpellPoints)cab1.Char.QuickRefSpellPoints).MaximumSP - ((MMSpellPoints)cab2.Char.QuickRefSpellPoints).MaximumSP));
                    return Order(String.Compare(cab1.Char.QuickRefSpellPoints.Current, cab2.Char.QuickRefSpellPoints.Current));
                case 8:
                    if (cab1.Char is BTCharacter)
                        return Order(String.Compare(((ListViewItem)x).SubItems[m_column].Text, ((ListViewItem)y).SubItems[m_column].Text));
                    return Order(Math.Sign(cab1.Char.BasicAge.Years - cab2.Char.BasicAge.Years));
    ...
    public class EOB1RosterFile : RosterFile
    {
        public static int GetExpectedSize() { return 0; }
        public static bool AcceptableSize(long iSize) { return iSize == GetExpectedSize(); }
        public override int ExpectedSize { get { return GetExpectedSize(); } }

        public EOB1RosterFile(string strFileName, bool bSilent)
        {
            Game = GameNames.EyeOfTheBeholder1;
        }
    }

    public class EOB2RosterFile : RosterFile
    {
        public static int GetExpectedSize() { return 0; }
        public static bool AcceptableSize(long iSize) { return iSize == GetExpectedSize(); }
        public override int ExpectedSize { get { return GetExpectedSize(); } }

        public EOB2RosterFile(string strFileName, bool bSilent)
        {
