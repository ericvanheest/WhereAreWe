...
namespace WhereAreWe
{
    public partial class AttributeEditForm : Form
    {
        ...
        public void UpdateUI(Array values, long max, long min)
        {
            ...
            sb.AppendFormat("{0}", values.GetValue(0));
            tbVal1.Text = values.GetValue(0).ToString();
            if (values.Length > 1)
            {
                sb.AppendFormat("/{0}", values.GetValue(1));
                tbVal2.Text = values.GetValue(1).ToString();
            }
            if (values.Length > 2)
            {
                sb.AppendFormat("/{0}", values.GetValue(2));
                tbVal3.Text = values.GetValue(2).ToString();
            }
