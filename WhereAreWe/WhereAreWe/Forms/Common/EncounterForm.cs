...
namespace WhereAreWe
{
    public partial class EncounterForm : HackerBasedForm
    {
        ...
        public bool UpdateUI(EncounterInfo info, bool bForce = false)
        {
            ...
            if (Hacker == null || info is MM345EncounterInfo && (DateTime.Now - m_dtLastUpdate).TotalMilliseconds < 400)
            {
                if (bForce)
            ...
            bool bUseProximity = (info is MM345EncounterInfo);
            ...
            foreach (Monster monster in info.Monsters.Values)
            {
                ...
                if ((!monster.Killed || miMonsterShowOffMap.Checked) && !bHideBit)
                {
                    ...
                    if (bShow)
                    {
                        ...
                        if (m_prevSelected.Contains(monster.EncounterIndex))
                            lvi.Selected = true;
                        lviList.Add(lvi);
                    }
                }
            ...
            if (m_iLastSortColumn != -1)
            {
                lvMonsters.ListViewItemSorter = new EncounterItemComparer(m_iLastSortColumn, m_bAscendingSort);
                lvMonsters.Sort();
            }
        ...
        private void UpdateMonsterLVI(ListViewItem lvi, bool bRanged, bool bPreEncounter, Monster monster, string strPrefix, Proximity prox)
        {
            ...
            lvi.SubItems[iSubItem++].Text = monster.Accuracy.ToString();
        ...
        {
            Properties.Settings.Default.EncounterListColumns = new ColumnHeaderList(lvMonsters);
            timerRefreshInfo.Stop();
        ...
        private void timerRefreshInfo_Tick(object sender, EventArgs e)
        {
            ...
            else if (Hacker.Running)
            {
                ...
                {
                    m_colWidths = new ColumnHeaderList(lvMonsters);
        ...
        class EncounterItemComparer : IComparer
        {
            private int m_column;
            private bool m_bAscending;

            public EncounterItemComparer()
            {
                m_column = 0;
                m_bAscending = true;
            }

            public EncounterItemComparer(int column, bool bAscending)
            {
                m_column = column;
            ...
            public int Compare(object x, object y)
            {
                ...
                switch (m_column)
                {
                    case 0:
                        if (!Global.Debug)
                        {
                            returnVal = String.Compare(item1.Monster.ProperName, item2.Monster.ProperName);
                            if (returnVal == 0)
                                returnVal = Math.Sign(item1.Monster.EncounterIndex - item2.Monster.EncounterIndex);
                        }
                        else
                            returnVal = Math.Sign(item1.Monster.EncounterIndex - item2.Monster.EncounterIndex);
                        break;
                    case 1:
                        returnVal = Math.Sign(item1.Prox.Full - item2.Prox.Full);
                        break;
                    case 2:
                        returnVal = String.Compare(Global.PointString(item1.Monster.Position), Global.PointString(item2.Monster.Position));
                        break;
                    case 3:
                        returnVal = Math.Sign(item1.Monster.CurrentHP - item2.Monster.CurrentHP);
                        break;
                    case 4:
                        returnVal = Math.Sign(item1.Monster.AC - item2.Monster.AC);
                        break;
                    case 5:
                        returnVal = Math.Sign(item1.Monster.AverageDamage - item2.Monster.AverageDamage);
                        break;
                    case 6:
                        returnVal = Math.Sign(item1.Monster.Speed - item2.Monster.Speed);
                        break;
                    case 7:
                        returnVal = Math.Sign(item1.Monster.Accuracy - item2.Monster.Accuracy);
                        break;
                    case 8:
                        returnVal = Math.Sign(item1.Monster.AverageResistance - item2.Monster.AverageResistance);
                        break;
                    case 9:
                        returnVal = String.Compare(item1.Monster.AllPowersString, item2.Monster.AllPowersString);
                        break;
                    case 10:
                        returnVal = Math.Sign(item1.Monster.Experience - item2.Monster.Experience);
                        break;
                    case 11:
                        returnVal = Math.Sign(item1.Monster.TreasureStrength - item2.Monster.TreasureStrength);
                        break;
                    case 12:
                        returnVal = String.Compare(item1.Monster.TargetString, item2.Monster.TargetString);
                        break;
                    default:
                        break;
                }
        ...
        private void lvMonsters_ColumnClick(object sender, ColumnClickEventArgs e)
        {
            ...
            lvMonsters.ListViewItemSorter = new EncounterItemComparer(e.Column, m_bAscendingSort);
            lvMonsters.Sort();
        }
        ...
        private void miMonsterTeleportParty_Click(object sender, EventArgs e)
        {
            ...
            MM345Monster monster = (lvMonsters.FocusedItem.Tag as EncounterItemTag).Monster as MM345Monster;
            if (monster == null)
                return;
