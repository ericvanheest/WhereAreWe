...
namespace WhereAreWe
{
    ...
    public class BT1QuestInfo : QuestInfo
    {
        ...
        {
            return GetQuest<BT1Quest>(status, type, bits, name, giver, reward, null, locations);
        }

        public BasicQuest AddSideQuest(QuestStatus status, List<BT1Quest> quests, string strReward = "", string strPath = "")
        {
            BT1Quest quest = GetQuest(status, BasicQuestType.Side, null, null, strReward) as BT1Quest;
        ...
        public BasicQuest AddMainQuest(QuestStatus status, List<BT1Quest> quests, string strReward = "")
        {
            return AddQuest(BasicQuestType.Primary, status, quests, null, String.Empty, strReward, String.Empty);
        }

        public BasicQuest AddMainQuest(int iSortOrder, QuestStatus status, List<BT1Quest> quests, string strReward = "")
        {
        ...
        public BasicQuest AddQuest(BasicQuestType type, QuestStatus status, List<BT1Quest> quests, object bits, string strGiver, string strReward, string strPath)
        {
            BT1Quest quest = null;
            if (status.Main != QuestStatus.Basic.InvalidClass)
            {
                quest = GetQuest(status, type, bits, strGiver, strReward) as BT1Quest;
                quest.Path = strPath;
                quests.Add(quest);
            }
            return quest;
