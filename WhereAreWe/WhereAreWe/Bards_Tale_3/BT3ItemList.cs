...
namespace WhereAreWe
{
    ...
    public class BT3Item : BTItem
    {
        ...
        public void SetBT3Bytes(int index, byte[] bytes, int offset = 0)
        {
            ...
            if (bytes[4] == 0 || !base.IsWeapon)
                ...
                Damage = new DamageDice(iFaceIndex == 0 ? 2 : iFaceIndex * 4, (bytes[4] & 0x0f) + 1, (bytes[0] & 0xf0) >> 4);
        ...
        public static byte GetEffectByte(BT2ItemEffect effect, BT3SpellIndex spell)
        {
        ...
        public static string GetName(BT3ItemIndex index)
        {
            switch (index)
            {
                ...
                case BT3ItemIndex.ArwsOfLife: return "Arrows of Life";
        ...
        public override string UseEffectString
        {
            get
            {
                ...
                if (ItemEffect != BT2ItemEffect.None)
                    sb.AppendFormat("{0}; ", BT2Item.GetEffectString(ItemEffect));
    ...
    public class BT3ShopInventory : ShopInventory
    {
        ...
        public BT3ShopInventory(byte[] bytes, int offset = 0)
        {
            ...
            {
                if (bytes[i] >= BT3.Items.Count)
                {
                    Items = null;
                    return;
                }

                Items.Add(new ShopItem(Global.BT3.GetClonedItem(bytes[i]), i, -1));
            }
        }
    }
}
