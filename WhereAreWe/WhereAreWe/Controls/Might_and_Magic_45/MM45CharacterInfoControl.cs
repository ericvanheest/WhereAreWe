    ...
    public partial class MM45CharacterInfoControl : MMCharacterInfoControl
    {
        ...
        public override void UpdateUI()
        {
            ...
            if (m_bDebugMonitorBackpack)
            {
                ...
                for(int i = 0; i < Math.Max(m_lastBackpack.Count, mm4Char.Inventory.Items.Count); i++)
                {
                    if (i >= m_lastBackpack.Count)
                        Global.Log("Added Item: {0}", mm4Char.Inventory.Items[i].DescriptionString);
                    else if (i >= mm4Char.Inventory.Items.Count)
                        Global.Log("Removed Item: {0}", m_lastBackpack[i].DescriptionString);
                    else if (m_lastBackpack[i].DescriptionString != mm4Char.Inventory.Items[i].DescriptionString)
                        Global.Log("Changed Item: {0} <=> {1}", m_lastBackpack[i].DescriptionString, mm4Char.Inventory.Items[i].DescriptionString);
                }
                //if (mm4Char.Inventory.Items.Count >= 9)
                //    Hacker.SetBackpack(mm4Char.BasicAddress, new List<Item>());
                m_lastBackpack = mm4Char.Inventory.Items;
            }
