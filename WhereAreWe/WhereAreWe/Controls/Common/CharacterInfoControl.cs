...
namespace WhereAreWe
{
    public partial class CharacterInfoControl : HackerBasedUserControl
    {
        ...
        public void SetInventoryLVI(ListView lv, int index, Item item, BaseCharacter character)
        {
            ...
            if (item == null || (item.Index == 0 && !(item is WizItem)))
            {
            ...
            else
            {
                ...
                if (!item.IsIdentified && !item.RevealUnidentified && Properties.Settings.Default.HideUnidentifiedItems)
                    lv.Items[index].ToolTipText = Global.UnidentifiedItemTip;
                else
        ...
        public static TriggerTarget[] CheckTriggerConditions(BaseCharacter baseChar, CharacterTrigger trigger)
        {
            ...
            foreach (TriggerCondition cond in trigger.Conditions)
            {
                ...
                {
                    if (CheckTriggerCondition(baseChar, tc))
                        listTested.Add(new TriggerTarget(tc.What, tc.WhatValue));
        ...
        public virtual TriggerControl[] CheckTrigger(CharacterTrigger trigger)
        {
            ...
            TriggerTarget[] testedEntities = CheckTriggerConditions(m_char, trigger);
            ...
            foreach (TriggerTarget target in targets)
            {
                ...
                switch (trigger.Do)
                {
                    ...
                    case TriggerDo.SetColorTo:
                        if (ctrl.Main is ListView)
                            Global.SetListViewSubItemColor(ctrl.Main as ListView, ctrl.Index, ctrl.SubIndex, trigger.DoColorFore, trigger.DoColorBack);
                        else if (ctrl.Main is TabPage)
                            m_main.PartyForm.SetTabStyle(ctrl.Main as TabPage, trigger);
                        else
                        {
                            ctrl.Main.ForeColor = trigger.DoColorFore;
                            ctrl.Main.BackColor = trigger.DoColorBack;
                        }
                        break;
                    default:
                        break;
        ...
        {
            int iIndex = 0;
            if (Int32.TryParse(strSearch, out iIndex))
                return iIndex;
        ...
        {
            long iVal = 0;
            Int64.TryParse(strVal, out iVal);
        ...
        public static string StringValue(BaseCharacter baseChar, TriggerEntity entity, string strVal)
        {
            switch (entity)
            {
        ...
        public static bool CheckTriggerCondition(BaseCharacter baseChar, TriggerCondition cond)
        {
            ...
            string strSource = StringValue(baseChar, cond.What, cond.WhatValue);
            string strTarget = StringValue(baseChar, cond.Which, cond.WhichValue);

            long iSource = 0;
            long iTarget = 0;
            bool bValidSource = Int64.TryParse(strSource, out iSource);
            bool bValidTarget = Int64.TryParse(strTarget, out iTarget);
            bool bNumbers = bValidSource && bValidTarget;
        ...
        private void ModifySelectedValue(CheatMenuFlags flags)
        {
            ...
            switch (flags)
            {
                ...
                case CheatMenuFlags.SuperChar:
                    ...
                    if (MessageBox.Show("This will maximize all of this character's stats and stock the inventory with some of the best items.  Proceed?",
        ...
        private void miBackpackBagofHolding_Click(object sender, EventArgs e)
        {
            ...
            if (Hacker is Wiz4MemoryHacker)
            {
                ...
                MessageBox.Show("Wizardry 4 does not have a roster of characters, so there is no Bag of Holding.\r\n\r\n" + strExtra,
        ...
        protected bool HighlightInventory(ItemType type, bool bUpdate = true)
        {
            ...
            foreach (ListViewItem lvi in m_commonCtrls.lvEquipped.Items)
            {
                ...
                if (iit != null && iit.Item.Matches(type))
                {
    ...
    public abstract class BaseCharacter
    {
        ...
        public void SetFromBytes(int iCharIndex, byte[] bytes, GameInfo info, EncounterInfo encounterInfo = null)
        {
            SetCharFromStream(iCharIndex, new MemoryStream(bytes), info, encounterInfo);
        }
        ...
        public virtual void SetCharFromStream(int iCharIndex, Stream stream, GameInfo info, EncounterInfo encounterInfo = null, bool bFromRosterFile = false) { }
        ...
        public StatModifier GetModifier(int iValue, PrimaryStat stat)
        {
    ...
    public class PermAndTemp
    {
        ...
        public string SlashString { get { return String.Format("{0}/{1}", Temporary, Permanent); } }
    ...
    {
        public OneByteStat(byte[] bytes, int indexTemp, int indexPerm) : base(bytes[indexTemp], bytes[indexPerm]) { }
