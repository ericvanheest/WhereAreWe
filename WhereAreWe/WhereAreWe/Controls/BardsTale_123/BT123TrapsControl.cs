...
{
    public partial class BT123TrapsControl : TrapsControl
    {
        ...
        public BT123TrapsControl()
        {
            InitializeComponent();
            Init();
        }

        public BT123TrapsControl(IMain main)
        {
            InitializeComponent();
            SetMain(main);
            Init();
        ...
        public override void SetSearchResults(SearchResults results)
        {
            ...
            {
                BT3Character btChar = ((CharTag) comboCharacter.Items[i]).Character as BT3Character;
                if (btChar == null)
                    continue;
                int iDisarm = btChar.Class == BT3Class.Rogue ? btChar.Disarm : btChar.Level;
            ...
            if (results is BTSearchResults)
            {
                // Select the first possible trap type
                int trap = ((BTSearchResults)results).Trap;
        ...
        public override bool DisarmTrap()
        {
            ...
            BTGameState state = Hacker.GetGameState() as BTGameState;
