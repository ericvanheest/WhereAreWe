...
{
    public partial class BT123TrainingAssistantControl : TrainingAssistantControl
    {
        ...
        public BT123TrainingAssistantControl()
        {
            InitializeComponent();
            Init();
        }

        public BT123TrainingAssistantControl(IMain main)
        {
            InitializeComponent();
            SetMain(main);
            Init();
        ...
        private void UpdateUI(TrainingInfo info, bool bForce)
        {
            if (info is BTTrainingInfo)
                UpdateUI((BTTrainingInfo)info, bForce);
        }

        private void AddCharacter(BTCharacter btChar, int index, int iLowHP, int iHighHP, int iLowSP, int iHighSP)
        {
            ...
            lvi.SubItems.Add(String.Format("{0}", btChar.Stats.IQ.Permanent));
        ...
        private void UpdateUI(BTTrainingInfo info, bool bForce)
        {
            ...
            BTPartyInfo btInfo = info.Party as BTPartyInfo;
            if (btInfo == null)
                return;
            ...
            for (int iChar = 0; iChar < info.Party.NumChars; iChar++)
            {
                ...
                BTCharacter btChar = BTCharacter.Create(Game, iChar, btInfo.Bytes, Games.CharacterSize(Game) * iAddress);

                int iBonusHP = Hacker.GetStatModifier(btChar.Stats.Constitution.Permanent, PrimaryStat.Constitution).Value;
                int iBonusSP = Hacker.GetStatModifier(btChar.Stats.IQ.Permanent, PrimaryStat.IQ).Value;
                ...
                {
                    BTCharacter btCharPrev = BTCharacter.Create(Game, iChar, m_bytesPrevious, Games.CharacterSize(Game) * iAddress);
