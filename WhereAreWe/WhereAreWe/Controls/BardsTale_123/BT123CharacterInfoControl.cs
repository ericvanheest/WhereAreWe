    ...
    public partial class BT123CharacterInfoControl : CharacterInfoControl
    {
        ...
        public override void UpdateUI()
        {
            ...
            if (m_bDebugMonitorBackpack)
            {
                ...
                for (int i = 0; i < Math.Max(m_lastBackpack.Count, btChar.Inventory.Items.Count); i++)
                {
                    if (i >= m_lastBackpack.Count)
                        Global.Log("Char{0} +Item: {1}", btChar.Address, btChar.Inventory.Items[i].DebugString);
                    else if (i >= btChar.Inventory.Items.Count)
                        Global.Log("Char{0} -Item: {1}", btChar.Address, m_lastBackpack[i].DebugString);
                    else if (m_lastBackpack[i].DebugString != btChar.Inventory.Items[i].DebugString)
                        Global.Log("Char{0} xItem: {1} <=> {2}", btChar.Address, m_lastBackpack[i].DebugString, btChar.Inventory.Items[i].DebugString);
                }
                m_lastBackpack = btChar.Inventory.Items;
            }
