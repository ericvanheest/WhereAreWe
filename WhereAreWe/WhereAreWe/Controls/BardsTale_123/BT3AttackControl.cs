...
{
    public partial class BT3AttackControl : UserControl
    {
        public BT3AttackControl()
        {
            InitializeComponent();
            if (comboAttack.Items.Count == 0)
            {
                comboAttack.Items.Add("Melee Attack");
                foreach (BT3Spell spell in BT3.Spells)
        ...
        public BT3MonsterAttack Attack
        {
            get
            {
                if (m_bytesOriginal == null)
                    return new BT3MonsterAttack(new byte[] { 0, 0 });
                if (comboAttack.SelectedIndex == 0)
                    return new BT3MonsterAttack(new byte[] { m_bytesOriginal[0] < 0x80 ? (byte) 0x80 : m_bytesOriginal[0], DamageByte });
                return new BT3MonsterAttack(new byte[] { (byte)(comboAttack.SelectedIndex - 1), (byte)nudLevel.Value });
            }
