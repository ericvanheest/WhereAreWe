...
namespace WhereAreWe
{
    ...
    public class MapSquare
    {
        ...
        public bool HasIcon(IconName name)
        {
            return Icons != null && Icons.Any(i => i.Name == name);
        }

        public void AddUniqueIcon(MapIcon icon)
        {
            if (icon == null || icon.Name == IconName.None)
                return;

            if (HasIcon(icon.Name, icon.Orientation))
                return;

            // Some icons are technically unique but shouldn't be doubled up
            switch (icon.Name)
            {
        ...
        public void RemoveEmptyIcons()
        {
            int i = Icons.Count - 1;
            while (i > -1)
            {
                if (Icons[i].Name == IconName.None)
                    Icons.RemoveAt(i);
                i--;
            }
        }
        ...
        public static Point OffsetForIcon(MapIcon icon, Rectangle rc)
        {
            // Some orientations look better with the 24x24 or 48x48 icon size if they are pushed one more pixel outward
            bool b24 = rc.Width >= 24 && rc.Width < 32;
            bool bLarge = rc.Width > 32;

            bool b0 = icon.Orientation == Direction.Up;
            bool b270 = icon.Orientation == Direction.Left;
            bool b90 = icon.Orientation == Direction.Right;
            bool b180 = icon.Orientation == Direction.Down;

            int iWidth = rc.Width / 48;
            int iHeight = rc.Height / 48;

            switch (icon.Name)
            {
                case IconName.Corner:
                case IconName.RotCorner:
                case IconName.Slash:
                case IconName.Bar:
                case IconName.CurveLeft:
                case IconName.CurveRight:
                case IconName.X:
                case IconName.Cross:
                case IconName.Y:
                case IconName.ArrowCap:
                case IconName.ArrowCapDiag:
                    return new Point(iWidth, iHeight);
                default:
                    return b24 || bLarge ? new Point(b90 ? iWidth + 1 : 0, b180 ? iHeight + 1 : 0) : Point.Empty;
            }
        }
        ...
        public static Bitmap GetDrawIconBitmap(Graphics g, MapIcon icon, ref Rectangle rc, EditFlags flags)
        {
            ...
            rc.Offset(OffsetForIcon(icon, rc));
            return bmpIcon;
        }
        ...
        public bool HasSightBlockingLine(GameNames game, MapSheet sheet, Direction dir, MapLineInfo gridLines, bool bForward = true)
        {
            switch (dir)
            {
                case Direction.Down: return Games.IsSightBlocking(game, sheet, gridLines, Bottom, Icons, dir, bForward);
                case Direction.Left: return Games.IsSightBlocking(game, sheet, gridLines, Left, Icons, dir, bForward);
                case Direction.Right: return Games.IsSightBlocking(game, sheet, gridLines, Right, Icons, dir, bForward);
                case Direction.Up: return Games.IsSightBlocking(game, sheet, gridLines, Top, Icons, dir, bForward);
                default: break;
            }
            return false;
        }
    ...
    public class MapBook
    {
        ...
        public ItemLocations TranslateLocationToMap(ItemLocations items, MapSheet sheet)
        {
            ItemLocations itemsNew = new ItemLocations();

            foreach (ItemPosition item in items.ItemPositions.Values)
            {
                ItemPosition pos = new ItemPosition(item);
                pos.Position = TranslateLocationToMap(pos.Position, sheet);
                itemsNew.ItemPositions.Add(pos.Position, pos);
            }
        ...
        public string Text
        {
            get
            {
                switch (Name)
                {
                    ...
                    case IconName.Slash: return "Slash";
                    case IconName.Corner: return "Corner";
                    case IconName.RotCorner: return "Diag Corner";
                    case IconName.CurveLeft: return "Curve Left";
                    case IconName.CurveRight: return "Curve Right";
                    case IconName.Bar: return "Bar";
                    case IconName.X: return "X";
                    case IconName.Cross: return "Cross";
                    case IconName.Y: return "Y";
                    case IconName.ArrowCap: return "Arrow Cap";
                    case IconName.ArrowCapDiag: return "Arrow Cap Diag";
                    default: return "<none>";
                }
            }
        }
        ...
        public Icon Image
        {
            get
            {
                switch (Name)
                {
                    ...
                    case IconName.Slash: return Properties.Resources.iconSlash;
                    case IconName.Corner: return Properties.Resources.iconWestToNorth;
                    case IconName.RotCorner: return Properties.Resources.iconNorthwestToNorthEast;
                    case IconName.CurveLeft: return Properties.Resources.iconNorthwestToSouth;
                    case IconName.CurveRight: return Properties.Resources.iconNortheastToSouth;
                    case IconName.Bar: return Properties.Resources.iconBar;
                    case IconName.X: return Properties.Resources.iconX;
                    case IconName.Cross: return Properties.Resources.iconCross;
                    case IconName.Y: return Properties.Resources.iconY;
                    case IconName.ArrowCap: return Properties.Resources.iconArrowCap;
                    case IconName.ArrowCapDiag: return Properties.Resources.iconArrowCapDiag;
